...
const CONFIG = {
  ...
  // Battle Settings - Vertical arena for mobile
  BATTLE: {
    arenaWidth: 320,
    arenaHeight: 480,
    tickRate: 58, // 15% slower than original 50ms
    moveSpeed: 2,
...
const BattleEngine = {
  createBattleState: (monster1, monster2) => {
    ...
    return {
      fighters: [
        {
          // Player monster - starts at bottom center
          monster: monster1,
          stats: stats1,
          x: CONFIG.BATTLE.arenaWidth / 2,
          y: CONFIG.BATTLE.arenaHeight - 70,
        ...
        {
          // Enemy monster - starts at top center
          monster: monster2,
          stats: stats2,
          x: CONFIG.BATTLE.arenaWidth / 2,
          y: 70,
  ...
  makeDecision: (fighter, enemy, tick, projectiles) => {
    ...
    // Only dodge if not on cooldown and projectile is close
    const canDodge = now - fighter.lastDodge > CONFIG.BATTLE.dodgeCooldown;
    if (canDodge && incomingProjectiles.length > 0 && Math.random() < (0.4 + intelligenceBonus)) {
      return { action: 'dodge', reason: 'projectile' };
    }
    
    // Attack decision - ALWAYS try to attack if able and in range
    // Agility reduces attack cooldown (faster attacks)
    const agilitySpeedBonus = 1 - (fighter.stats.agility * 0.02); // Up to 60% faster at 30 AGI
    ...
    // Attack if in range and can attack - HIGH priority
    if (canAttack && distance < effectiveRange) {
      // Always attack when in range, no random chance
      return { action: 'attack', useMagic: useMagic };
    }
    
    // Approach if out of range - this is the main movement goal
    if (distance > effectiveRange * 0.7) {
    ...
    if (canDodge && enemy.state === 'attacking' && distance < CONFIG.BATTLE.attackRange * 1.5) {
      if (Math.random() < (0.35 + intelligenceBonus)) {
        return { action: 'dodge', reason: 'melee' };
      }
    }
    
    // If can't attack yet and in range, circle/strafe while waiting for cooldown
    if (!canAttack && distance < effectiveRange) {
      if (Math.random() < 0.6) {
    ...
    // Retreat if too close and ranged fighter (but not if melee)
  ...
  processTick: (battleState) => {
    ...
    newState.projectiles = newState.projectiles.filter(proj => {
      ...
      if (dist < CONFIG.BATTLE.monsterRadius + proj.size) {
        ...
        if (target.dodgeEndTime && now < target.dodgeEndTime) {
          ...
          // Knockback - INCREASED for more impact
          ...
          const baseKnockback = 6 + (attackStat / 3); // Increased base knockback
          const finalKnockback = Math.max(4, baseKnockback * (1 - defenseReduction * 0.4)); // Min 4 knockback
          ...
          newState.log.push(`${shooter.monster.name} blasted ${target.monster.name} for ${damage}!`);
        }
        return false; // Remove projectile
      }
    ...
    [f1, f2].forEach(fighter => {
      const r = CONFIG.BATTLE.monsterRadius;
      const bounceForce = 0.6; // How much velocity is preserved on bounce
      const intelligence = fighter.stats.intelligence || 10;
      const smartBounce = Math.min(0.3, intelligence / 50); // Smart monsters recover better
      
      // Left wall
      if (fighter.x < r) {
        fighter.x = r;
        fighter.vx = Math.abs(fighter.vx) * (bounceForce + smartBounce);
        // Stun dumb monsters briefly when they hit walls
...
const MonsterSprite = ({ monster, size = 80, showStats = false, isInBattle = false, fighter = null, emotion = 'happy', showEmoji = true, forcedEmoji = null }) => {
  ...
  // Show emoji when emotion changes
  React.useEffect(() => {
    if (!showEmoji) return;
  ...
  }, [currentEmotion, showEmoji, forcedEmoji]);
  ...
  const getSpriteContent = () => {
    ...
    return (
      ...
      >
        {/* Floating emoji - appears above monster head like hearts */}
        {emojiVisible && currentEmoji && (
          <div 
            ...
            style={{
              ...
              fontSize: Math.max(20, spriteSize * 0.35),
            }}
          >
            {currentEmoji}
          </div>
      ...
      <div className="bg-gray-900 rounded-3xl p-6 max-w-lg w-full">
        ...
        <div className="bg-gray-800/50 rounded-xl p-4 mb-4">
          ...
          {['strength', 'constitution', 'agility', 'power', 'intelligence'].map(stat => {
            ...
            const isTie = playerVal === enemyVal;
            
            return (
              <div key={stat} className="grid grid-cols-3 gap-2 py-1.5 border-t border-gray-700/50">
          ...
          {/* Total/Power row */}
          <div className="grid grid-cols-3 gap-2 py-2 border-t-2 border-gray-600 mt-1">
            <div className={`text-center font-bold ${playerPower > enemyPower ? 'text-green-400' : playerPower < enemyPower ? 'text-red-400' : 'text-gray-400'}`}>
              {playerPower}
              {playerPower > enemyPower && <span className="ml-1 text-xs">▲</span>}
            </div>
            <div className="text-center text-yellow-400 text-xs flex items-center justify-center font-bold">
              ⚡ PWR
            </div>
            <div className={`text-center font-bold ${enemyPower > playerPower ? 'text-green-400' : enemyPower < playerPower ? 'text-red-400' : 'text-gray-400'}`}>
              {enemyPower}
              {enemyPower > playerPower && <span className="ml-1 text-xs">▲</span>}
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-800/50 rounded-xl p-3 mb-4">
          <p className="text-sm text-gray-400 text-center">
            Win: <span className="text-yellow-400">{CONFIG.LEVELING.expPerWin} EXP</span> + <span className="text-yellow-400">{CONFIG.LEVELING.trainingPointsPerWin} TP</span>
          </p>
          <p className="text-xs text-gray-500 text-center mt-1">
            Lose: <span className="text-gray-400">{Math.floor(CONFIG.LEVELING.expPerWin / 3)} EXP</span> (learn from defeat!)
          </p>
        </div>
...
const BattleResultModal = ({ result, onClose, onBattleAgain }) => {
  const { won, monster, expGained, trainingPointsGained, leveledUp, statGains = {}, oldStats = {}, newStats = {} } = result;
              ...
              <div className="bg-gray-800/50 rounded-xl p-4 mb-4">
                ...
                <div className="flex justify-around">
                  ...
                  <div className="text-center">
                    ...
                    <div className="text-xs text-gray-400">Training Point</div>
                  </div>
                </div>
              </div>
...
export default function MonsterGame() {
  ...
  const [battleResult, setBattleResult] = useState(null); // Post-battle result modal
  const [careEmotion, setCareEmotion] = useState('happy'); // Emotion for care interactions
  const [forcedEmoji, setForcedEmoji] = useState(null); // Force specific emoji to show
  ...
  const handleEndBattle = () => {
    ...
    if (battleState?.winner) {
      ...
      if (isPlayerWin && winnerMonster) {
        ...
        // Award experience and training points
        const expResult = MonsterFactory.addExperience(winnerMonster, CONFIG.LEVELING.expPerWin);
        let updatedMonster = MonsterFactory.addTrainingPoints(expResult.monster, CONFIG.LEVELING.trainingPointsPerWin);
        ...
        setBattleResult({
          ...
          expGained: CONFIG.LEVELING.expPerWin,
          trainingPointsGained: CONFIG.LEVELING.trainingPointsPerWin,
        ...
        // Find the player's monster for the loss screen
        ...
        if (playerMonster) {
          ...
          const newStats = MonsterFactory.getEffectiveStats(expResult.monster);
          
          // Update monster with loss XP
          setMonsters(prev => prev.map(m => m.id === playerMonster.id ? expResult.monster : m));
          
          setBattleResult({
            won: false,
            monster: expResult.monster,
            expGained: lossExp,
  ...
  return (
    ...
    >
      {/* Global styles for floating emoji animations */}
      <style>{`
      ...
      <div className="max-w-lg mx-auto px-4 py-4">
        {view === 'care' ? (
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-3 py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white text-sm"
                >
                  + New Monster
                </button>
              </div>
              ...
              {monsters.length === 0 ? (
                <div className="text-center py-8 text-gray-500 bg-gray-900/30 rounded-2xl">
                  ...
                  <p>No monsters yet!</p>
                  <p className="text-sm">Tap "+ New Monster" to start</p>
                </div>
              ) : (
            ...
            {selectedMonster ? (
              <div className="bg-gray-900/50 rounded-2xl p-4">
                <div className="flex items-center gap-4">
                  <div 
                    onClick={() => {
                      // Random happy reaction when clicked
                      const reactions = ['love', 'happy', 'excited'];
                      const emojis = ['💕', '😊', '✨', '🥰', '💖'];
                      triggerEmotion(
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-gray-500 text-xs mt-0.5">
                      📅 {MonsterFactory.getAgeString(selectedMonster)}
                    </p>
                    
                    {/* Body Type Badge with stat modifiers (only show for non-eggs) */}
                    {selectedMonster.stage !== 'EGG' && selectedMonster.bodyParts?.bodyType && (() => {
                      ...
                      return (
                        <div className="mt-1 flex items-center gap-2 flex-wrap">
                          <span className="inline-block px-2 py-0.5 text-xs rounded-full bg-purple-500/20 text-purple-300 border border-purple-500/30">
                            {bodyType?.name}
                          </span>
                    ...
                    {/* Evolution progress for eggs/infants */}
                    {selectedMonster.stage !== 'ADULT' && (
                      <div className="mt-2">
                        <div className="flex justify-between text-xs text-gray-400 mb-1">
                          <span>{selectedMonster.stage === 'EGG' ? 'Hatching' : 'Growing'}</span>
                          <span>{selectedMonster.age || 0}/{CONFIG.STAGES[selectedMonster.stage].duration}</span>
                        </div>
                        <div className="h-1.5 bg-gray-700 rounded-full overflow-hidden">
                          <div 
                            className="h-full bg-gradient-to-r from-purple-500 to-pink-500"
                            style={{ 
                              width: `${((selectedMonster.age || 0) / CONFIG.STAGES[selectedMonster.stage].duration) * 100}%` 
                            }}
                          />
                        </div>
                        <button
                          onClick={() => {
                            setMonsters(prev => prev.map(m => 
                              m.id === selectedMonster.id ? MonsterFactory.tickAge(m) : m
                            ));
                            // Update selectedMonster too
                            const updated = MonsterFactory.tickAge(selectedMonster);
                            setSelectedMonster(updated);
                            
                            // Trigger excited emoji on nurture/incubate
                            if (updated.stage !== selectedMonster.stage) {
                              // Evolution happened!
                          ...
                          className="mt-2 w-full py-2 rounded-lg bg-purple-600 active:bg-purple-500 text-white text-sm font-bold"
                        >
                          {selectedMonster.stage === 'EGG' ? '🥚 Incubate' : '✨ Nurture'}
                        </button>
                      </div>
                    )}
                    ...
                    {selectedMonster.stage === 'ADULT' && (
                      <div className="mt-2">
                        <div className="flex justify-between text-xs text-gray-400 mb-1">
                          <span>EXP</span>
                          <span>
                            {selectedMonster.exp || 0} / {MonsterFactory.getExpForLevel(selectedMonster.level || 1)}
                          </span>
                        </div>
                        <div className="h-1.5 bg-gray-700 rounded-full overflow-hidden">
                          <div 
                ...
                <div className="mt-4 bg-gray-800/30 rounded-xl p-3">
                  <h3 className="text-sm font-bold text-gray-300 mb-1" style={{ color: selectedMonster.type.color }}>
                    {selectedMonster.type.name} Type
                  </h3>
                  <p className="text-xs text-gray-400">
                    {selectedMonster.type.description}
                  </p>
                </div>
                
                {/* Personality Description (only for non-eggs) */}
                {selectedMonster.stage !== 'EGG' && selectedMonster.personality && (
                  <div className="mt-3 bg-gray-800/30 rounded-xl p-3">
                    <h3 className="text-sm font-bold text-gray-300 mb-1">Personality</h3>
                    <div className="flex flex-wrap gap-1 mb-2">
                      <span className="px-2 py-0.5 text-xs rounded-full bg-blue-500/20 text-blue-300 capitalize">
                        {selectedMonster.personality.energy}
                      </span>
                      <span className="px-2 py-0.5 text-xs rounded-full bg-red-500/20 text-red-300 capitalize">
                        {selectedMonster.personality.temperament}
                      </span>
                      <span className="px-2 py-0.5 text-xs rounded-full bg-green-500/20 text-green-300 capitalize">
                        {selectedMonster.personality.social}
                      </span>
                      <span className="px-2 py-0.5 text-xs rounded-full bg-yellow-500/20 text-yellow-300 capitalize">
                        {selectedMonster.personality.focus}
                      </span>
                    </div>
                    <p className="text-xs text-gray-400">
                      {MonsterFactory.getPersonalityDescription(selectedMonster)}
                    </p>
                  </div>
                )}
                
                {/* Training Points Info (only for non-eggs - infants can train!) */}
                {selectedMonster.stage !== 'EGG' && (
                  <div className="mt-3 bg-gray-800/30 rounded-xl p-3">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      {selectedMonster.stage === 'INFANT' 
                        ? 'Train your infant to shape its growth!' 
                        : 'Win battles to earn points. Train in the Train tab!'}
                    </p>
                  </div>
                )}
                ...
                {/* Egg waiting message */}
                {selectedMonster.stage === 'EGG' && (
                  <div className="mt-4 bg-gray-800/30 rounded-xl p-4 text-center">
                    <p className="text-gray-400 text-sm">
                      🥚 Your egg is incubating...
                    </p>
                    <p className="text-gray-500 text-xs mt-1">
                      Tap "Incubate" to help it hatch faster!
                    </p>
                  </div>
                )}
              </div>
                  ...
                  {CONFIG.TRAINING_TYPES.map(training => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-xs text-gray-400">{Math.round(currentStat)}</span>
          ...
          /* Battle View - Mobile optimized */
          <div className="space-y-4">
            {battleState ? (
              <BattleArena battleState={battleState} onEnd={handleEndBattle} />
            ) : (
              <>
                {/* Monster selector - horizontal scroll (adults only) */}
                <div className="space-y-3">
                  <h2 className="text-lg font-bold text-white">Select Fighter</h2>
                  {monsters.filter(m => m.stage === 'ADULT').length === 0 ? (
                    <div className="text-center py-8 text-gray-500 bg-gray-900/30 rounded-2xl">
                      <div className="text-4xl mb-2">🐣</div>
                      <p>No adult monsters!</p>
                      <p className="text-sm">Raise a monster to adulthood first</p>
                      <button
                        onClick={() => setView('care')}
                        className="mt-3 px-4 py-2 bg-purple-600 active:bg-purple-500 rounded-xl text-white text-sm"
                      >
                        Go to Care
                      </button>
                    </div>
                    ...
                    <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4">
                      {monsters.filter(m => m.stage === 'ADULT').map(monster => (
                        <div
                          key={monster.id}
                          onClick={() => handleSelectFighter(monster)}
                          className={`
                            flex-shrink-0 w-20 p-2 rounded-2xl cursor-pointer transition-all
                            ${selectedFighter?.id === monster.id 
                        ...
                        >
                          ...
                          <p className="text-xs text-gray-400 text-center">
                            {Math.round(Object.values(MonsterFactory.getEffectiveStats(monster)).reduce((a, b) => a + b, 0))} PWR
                          </p>
                        </div>
                      ))}
                    </div>
                ...
                {/* Battle Arena Placeholder / Fighter display */}
                {selectedFighter ? (
                  <div className="bg-gray-900/50 rounded-2xl p-4 text-center">
                    <MonsterSprite monster={selectedFighter} size={80} />
                    <h3 className="font-bold text-white mt-2">{selectedFighter.name}</h3>
                    <p className="text-sm text-gray-400">Ready to fight!</p>
                    <p className="text-xs text-gray-500 mt-1">Finding opponent...</p>
                  </div>
                ) : (
                  <div className="bg-gray-900/50 rounded-2xl p-8 text-center">
                    <div className="text-4xl mb-2">⚔️</div>
                    <p className="text-gray-400 text-sm">Select a monster to battle</p>
                  </div>
                )}
              </>
            )}
        ...
        <div className="max-w-lg mx-auto px-4">
          <div className="flex justify-around py-2">
            ...
            <button
              ...
              className={`flex flex-col items-center py-2 px-4 rounded-xl transition-all ${
                view === 'battle' ? 'text-red-400' : 'text-gray-500'
              }`}
            >
