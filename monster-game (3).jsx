...
const CONFIG = {
  ...
  LEVELING: {
    ...
    maxLevel: 50,
    trainingPointsPerLevel: 5, // Training points earned per level up (spend in Training tab)
  ...
  BATTLE: {
    ...
    moveSpeed: 2,
    attackRange: 60, // Melee range (increased from 50)
  ...
  STAT_EFFECTS: {
    strengthDamageMultiplier: 0.4, // Reduced for longer battles
    constitutionDefenseMultiplier: 0.5, // Buffed defense
    ...
    powerMagicDamageMultiplier: 0.4, // Reduced for longer battles
  ...
  HAPPINESS: {
    winBonus: 15,
    lossReduction: 20,
    recoveryPerTick: 3, // Recovery over time (3% every 3 seconds - faster!)
...
const Utils = {
  ...
  // Generate trash talk based on rival history
  generateTrashTalk: (rival, trainerName = 'you') => {
    if (!rival) {
      // First meeting - no trash talk
      return null;
    }
    
    const { wins, losses, encounters, name } = rival;
    const playerWins = losses; // Rival's losses = player's wins against them
    const playerLosses = wins; // Rival's wins = player's losses against them
    
    // Check if this is the #1 nemesis (passed as parameter)
    const isNemesis = rival.isNemesis || false;
    
    // NEMESIS-SPECIFIC DIALOGUE (only for #1 rival)
    if (isNemesis && encounters >= 3) {
      const nemesisDominatingTaunts = [
        `YOU! My greatest rival! I've crushed you ${playerLosses} times and I'll do it again!`,
        `${trainerName}... we meet again. You know how this ends.`,
        `There you are, my nemesis! Ready for defeat number ${playerLosses + 1}?`,
        `I've been waiting for YOU specifically. Let's settle this!`,
        `My #1 enemy returns! This rivalry ends TODAY!`,
      ];
      
      const nemesisLosingTaunts = [
        `${trainerName}!! You've beaten me ${playerWins} times... NO MORE!`,
        `My nemesis... I've trained JUST to defeat you!`,
        `Every loss to you has made me STRONGER! Watch this!`,
        `You're the one trainer I REFUSE to keep losing to!`,
        `I dream about beating you. Today it becomes reality!`,
      ];
      
      const nemesisEvenTaunts = [
        `${trainerName}! We're perfectly matched at ${playerWins}-${playerLosses}!`,
        `My greatest rival... one of us must prove superiority!`,
        `YOU! The trainer who haunts my dreams! Let's GO!`,
        `Our legendary rivalry continues! ${encounters} battles and counting!`,
        `Face me, nemesis! This is what we live for!`,
      ];
      
      let pool;
      if (playerLosses > playerWins + 1) {
        pool = nemesisDominatingTaunts;
      } else if (playerWins > playerLosses + 1) {
        pool = nemesisLosingTaunts;
      } else {
        pool = nemesisEvenTaunts;
      }
      
      return pool[Math.floor(Math.random() * pool.length)];
    }
    
    // Regular rival trash talk
    const dominatingTaunts = [
      `I've beaten you ${playerLosses} times! Ready to lose again?`,
      `Oh, it's you again! This will be quick.`,
      `Back for more punishment? I admire your spirit!`,
      `You just don't learn, do you? Let's make it ${playerLosses + 1}!`,
      `My ${rival.lastMonsterName || 'monster'} is hungry for another win!`,
    ];
    
    const losingTaunts = [
      `You got lucky ${playerWins} times! That ends NOW!`,
      `I've been training hard since our last fight!`,
      `This time will be different, I promise you!`,
      `Enjoy your winning streak while it lasts!`,
      `I've figured out your strategy. Prepare to lose!`,
    ];
    
    const evenTaunts = [
      `We're ${playerWins}-${playerLosses}... time to break the tie!`,
      `Evenly matched? Not for long!`,
      `One of us has to pull ahead. It's going to be me!`,
      `Our rivalry continues! May the best trainer win!`,
      `You know me, I know you. Let's settle this!`,
    ];
    
    const firstRematchTaunts = [
      `Back for a rematch already? I remember you!`,
      `So we meet again! This should be interesting.`,
      `I was hoping you'd come back. Let's go!`,
      `Round 2! Show me what you've got!`,
    ];
    
    let pool;
    if (encounters === 1) {
      pool = firstRematchTaunts;
    } else if (playerLosses > playerWins + 1) {
      pool = dominatingTaunts;
    } else if (playerWins > playerLosses + 1) {
      pool = losingTaunts;
    } else {
      pool = evenTaunts;
...
const MonsterFactory = {
  ...
  addExperience: (monster, exp) => {
    if (monster.level >= CONFIG.LEVELING.maxLevel) return { monster, leveledUp: false, statGains: {}, trainingPointsGained: 0 };
    ...
    // Grant training points on level up (5 points per level)
    ...
    return { monster: updated, leveledUp, levelsGained, trainingPointsGained, statGains: {} };
  },
...
const BattleEngine = {
  ...
  createFighter: (monster, x, y, facing = 1) => {
    ...
    // Base health increased, constitution gives more HP
  ...
  calculateDamage: (attacker, defender, isMagic = false) => {
    ...
    // Defense reduces damage with diminishing returns (can't reduce below 20%)
    const defense = defender.stats.constitution * CONFIG.STAT_EFFECTS.constitutionDefenseMultiplier;
    const defenseReduction = defense / (defense + 20); // Approaches 100% reduction asymptotically
    const finalDamage = baseDamage * 8 * (1 - defenseReduction * 0.8); // Max 80% reduction
  ...
  // AI decision making - Intelligence is the KEY stat for smart decisions
  makeDecision: (fighter, enemy, tick, projectiles, healOrbs = []) => {
    ...
    // INTELLIGENCE SCALING (no cap - higher INT = smarter decisions)
    // Uses diminishing returns formula: approaches 1 asymptotically but never caps
    const smartness = intelligence / (intelligence + 15); // 15 INT = 50%, 30 INT = 66%, 60 INT = 80%, etc.
    const randomChance = Math.max(0.02, 0.6 * (1 - smartness)); // Approaches 2% minimum randomness
    
    // Determine combat style
    const isMeleeOriented = strength > power * 1.2;
    const isRangedOriented = power > strength * 1.2;
    const isBalanced = !isMeleeOriented && !isRangedOriented;
    
    // Health awareness (smarter = more aware of health)
    const healthPercent = fighter.health / fighter.maxHealth;
    const isLowHealth = healthPercent < 0.25;
    const isCriticalHealth = healthPercent < 0.1;
    
    // CORNER AWARENESS - Smart monsters avoid getting trapped
    const margin = CONFIG.BATTLE.monsterRadius + 30;
    const arenaW = CONFIG.BATTLE.arenaWidth;
    const arenaH = CONFIG.BATTLE.arenaHeight;
    const nearLeftWall = fighter.x < margin;
    const nearRightWall = fighter.x > arenaW - margin;
    const nearTopWall = fighter.y < margin;
    const nearBottomWall = fighter.y > arenaH - margin;
    const isNearWall = nearLeftWall || nearRightWall || nearTopWall || nearBottomWall;
    const isInCorner = (nearLeftWall || nearRightWall) && (nearTopWall || nearBottomWall);
    
    // Smart monsters are VERY aware of being cornered
    const cornerDanger = isInCorner ? 1.0 : isNearWall ? 0.5 : 0;
    const shouldEscapeCorner = cornerDanger > 0 && Math.random() < (smartness * cornerDanger);
    
    // Calculate escape direction from walls/corners
    const getEscapeAngle = () => {
      let escapeX = arenaW / 2 - fighter.x; // Push toward center
      let escapeY = arenaH / 2 - fighter.y;
      // Normalize
      const mag = Math.sqrt(escapeX * escapeX + escapeY * escapeY) || 1;
      return Math.atan2(escapeY / mag, escapeX / mag);
    };
    
    // Agility affects attack cooldown (no hard cap - can get very fast with high agility)
    const agilitySpeedBonus = 1 - (agility * CONFIG.STAT_EFFECTS.agilityAttackSpeedMultiplier);
    const effectiveCooldown = CONFIG.BATTLE.attackCooldown * Math.max(0.15, agilitySpeedBonus); // Min 15% of base (was 40%)
    const canAttack = now - fighter.lastAttack > effectiveCooldown;
    const canDodge = now - fighter.lastDodge > CONFIG.BATTLE.dodgeCooldown;
    
    // =====================================================
    // PRIORITY 1: CORNER ESCAPE (Intelligence-based)
    // Smart monsters prioritize escaping dangerous positions
    // =====================================================
    if (shouldEscapeCorner && smartness > 0.3) {
      // Very smart: dodge out of corner if possible
      if (canDodge && smartness > 0.6) {
        return { action: 'dodge', reason: 'corner', escapeAngle: getEscapeAngle() };
      }
      // Otherwise move toward center
      return { action: 'escapeCorner', escapeAngle: getEscapeAngle() };
    }
    
    // =====================================================
    // PRIORITY 2: HEAL ORB AWARENESS (Intelligence-based)
    // Smarter monsters seek healing more effectively
    // =====================================================
    if (healOrbs.length > 0) {
      const healThreshold = 0.3 + smartness * 0.3; // Smart: heal at 60%, dumb: heal at 30%
      if (healthPercent < healThreshold) {
        const healAwareness = smartness * 0.8 + (isCriticalHealth ? 0.3 : 0);
        if (Math.random() < healAwareness) {
          // Smart monsters pick the BEST orb (closest + safest)
          let bestOrb = null;
          let bestScore = -Infinity;
          
          healOrbs.forEach(orb => {
            const orbDist = Utils.distance(fighter.x, fighter.y, orb.x, orb.y);
            const enemyToOrb = Utils.distance(enemy.x, enemy.y, orb.x, orb.y);
            // Score: prefer close orbs that enemy is far from
            const score = (200 - orbDist) + (smartness * enemyToOrb * 0.5);
            if (score > bestScore && orbDist < 180) {
              bestScore = score;
              bestOrb = orb;
            }
          });
          ...
          if (bestOrb) {
            return { action: 'seekHeal', target: bestOrb };
          }
        }
      }
    }
    ...
    // =====================================================
    // PRIORITY 3: PROJECTILE DODGING (Intelligence-based)
    // Smarter monsters react to incoming projectiles
    // =====================================================
    const incomingProjectiles = projectiles.filter(p => 
      p.targetId === fighter.monster.id &&
      Utils.distance(p.x, p.y, fighter.x, fighter.y) < 100
    );
    
    if (canDodge && incomingProjectiles.length > 0) {
      const dodgeChance = smartness * 0.9; // Up to 90% dodge chance for smart monsters
      if (Math.random() < dodgeChance) {
        // Smart monsters dodge perpendicular to projectile, dumb ones dodge randomly
        const proj = incomingProjectiles[0];
        const projAngle = Math.atan2(proj.vy, proj.vx);
        const smartDodgeAngle = projAngle + (Math.PI / 2) * (Math.random() > 0.5 ? 1 : -1);
        const dumbDodgeAngle = Math.random() * Math.PI * 2;
        const dodgeAngle = smartDodgeAngle * smartness + dumbDodgeAngle * (1 - smartness);
        return { action: 'dodge', reason: 'projectile', dodgeAngle };
      }
    }
    
    // =====================================================
    // PRIORITY 4: MELEE DODGE (Intelligence-based)
    // Smart monsters sidestep incoming melee attacks
    // =====================================================
    if (canDodge && enemy.state === 'attacking' && distance < CONFIG.BATTLE.attackRange * 1.5) {
      const meleeDodgeChance = smartness * 0.7;
      if (Math.random() < meleeDodgeChance) {
        return { action: 'dodge', reason: 'melee' };
      }
    }
    
    // =====================================================
    // COMBAT STYLE DECISIONS (Intelligence affects efficiency)
    // =====================================================
    
    // DUMB MONSTER RANDOM ACTION (low intelligence = random behavior)
    if (Math.random() < randomChance) {
      const randomActions = ['approach', 'circle', 'retreat', 'wander'];
      const action = randomActions[Math.floor(Math.random() * randomActions.length)];
      return { action, direction: Math.random() > 0.5 ? 1 : -1, isRandom: true };
    }
    ...
    if (isMeleeOriented) {
      ...
      // SMART: Charge directly, avoid walls
      // DUMB: Charge blindly, might run into walls
      if (distance > CONFIG.BATTLE.attackRange * 0.8) {
        // Smart monsters avoid charging into corners
        if (smartness > 0.5 && isNearWall) {
          // Angle charge to avoid wall
          return { action: 'chargeAngled', avoidWalls: true };
        }
        return { action: 'charge', aggressive: true };
      }
      
      // Close but can't attack - smart monsters circle strategically
      if (!canAttack && distance < CONFIG.BATTLE.attackRange * 1.3) {
        // Smart: circle toward open space, Dumb: random direction
        const toCenter = Math.atan2(arenaH/2 - fighter.y, arenaW/2 - fighter.x);
        const toEnemy = Math.atan2(enemy.y - fighter.y, enemy.x - fighter.x);
        const smartDir = Math.sin(toCenter - toEnemy) > 0 ? 1 : -1;
        const direction = smartness > 0.5 ? smartDir : (Math.random() > 0.5 ? 1 : -1);
        return { action: 'stayClose', direction };
      }
    ...
    // ----- RANGED ORIENTED -----
    if (isRangedOriented) {
      const idealRange = CONFIG.BATTLE.projectileRange * 0.6;
      const minSafeRange = CONFIG.BATTLE.attackRange * 2;
      
      // SMART: Kite effectively, maintain optimal range
      // DUMB: Panic retreat or stand still
      
      // Too close - need to create distance
      if (distance < minSafeRange) {
        // Smart: Kite toward open space, not into walls
        if (smartness > 0.4) {
          // Check if kiting would push into wall
          const kiteAngle = Math.atan2(fighter.y - enemy.y, fighter.x - enemy.x);
          const testX = fighter.x + Math.cos(kiteAngle) * 50;
          const testY = fighter.y + Math.sin(kiteAngle) * 50;
          
          if (testX < margin || testX > arenaW - margin || testY < margin || testY > arenaH - margin) {
            // Would hit wall - circle instead
            return { action: 'circle', direction: Math.random() > 0.5 ? 1 : -1, fast: true };
          }
        }
        return { action: 'kite', retreatDistance: minSafeRange - distance };
      }
      
      // In optimal range - attack!
      if (canAttack && distance < CONFIG.BATTLE.projectileRange && distance > minSafeRange * 0.7) {
        return { action: 'attack', useMagic: true };
      }
      
      // Too far - approach but maintain safety
      if (distance > CONFIG.BATTLE.projectileRange * 0.85) {
        return { action: 'approachCautious', targetDistance: idealRange };
      }
      
      // Waiting for cooldown - smart monsters reposition
      if (!canAttack) {
        // Smart: Circle to better position, Dumb: stand or wander
        if (smartness > 0.3) {
      ...
      return { action: 'kite', retreatDistance: 30 };
    }
    
    // ----- BALANCED (adapts based on situation + intelligence) -----
    
    // Smart balanced fighters choose optimal range based on situation
    const preferMelee = distance < CONFIG.BATTLE.attackRange * 1.5 || strength > power;
    
    // Low health + smart = play safe
    if (isLowHealth && smartness > 0.4) {
      if (distance < CONFIG.BATTLE.attackRange * 2) {
        return { action: 'kite', retreatDistance: 50 };
      }
    }
    
    // Attack if possible
    if (canAttack) {
      if (distance < CONFIG.BATTLE.attackRange * 1.1) {
        return { action: 'attack', useMagic: false };
      }
      if (distance < CONFIG.BATTLE.projectileRange && power >= strength * 0.7) {
        return { action: 'attack', useMagic: true };
      }
    }
    
    // Approach enemy
    if (distance > (preferMelee ? CONFIG.BATTLE.attackRange : CONFIG.BATTLE.projectileRange * 0.7)) {
      return { action: 'approach', cautious: isLowHealth && smartness > 0.3 };
    }
    
    // Close but cooling down - circle
    if (!canAttack) {
      const direction = smartness > 0.5 
        ? (Math.sin(Math.atan2(arenaH/2 - fighter.y, arenaW/2 - fighter.x) - Math.atan2(enemy.y - fighter.y, enemy.x - fighter.x)) > 0 ? 1 : -1)
        : (Math.random() > 0.5 ? 1 : -1);
      return { action: 'circle', direction };
    }
  ...
  processTick: (battleState) => {
    ...
    newState.projectiles = newState.projectiles.filter(proj => {
      ...
      if (dist < CONFIG.BATTLE.monsterRadius + proj.size) {
        ...
        if (target.dodgeEndTime && now < target.dodgeEndTime) {
          ...
          // Knockback (constitution reduces knockback with no cap)
          if (shooter) {
            const attackerStats = shooter.stats;
            const defenderStats = target.stats;
            const attackStat = attackerStats.power;
            // Diminishing returns on defense reduction - approaches 80% max reduction asymptotically
            const defenseReduction = defenderStats.constitution / (defenderStats.constitution + 20);
            const baseKnockback = 6 + (attackStat / 3);
            const finalKnockback = Math.max(2, baseKnockback * (1 - defenseReduction * 0.8));
    ...
    activeFighters.forEach((fighter) => {
      ...
      switch (decision.action) {
          ...
          if (decision.useMagic) {
            ...
            if (dist < CONFIG.BATTLE.attackRange * 1.3) {
              ...
              if (enemy.dodgeEndTime && now < enemy.dodgeEndTime) {
                ...
                // Calculate knockback - physical attacks have STRONGER knockback (no caps)
                ...
                // Diminishing returns on defense - approaches 80% max reduction
                const defenseRatio = defenderStats.constitution / (defenderStats.constitution + 20);
                const defenseReduction = 1 - (defenseRatio * 0.8);
                const knockbackForce = Math.max(3, (6 + attackStat / 2.5) * defenseReduction); // Stronger melee knockback
        ...
        case 'dodge':
          if (now - fighter.lastDodge > CONFIG.BATTLE.dodgeCooldown) {
            ...
            // Agility affects dodge speed and duration (no cap - scales infinitely)
            const agilityBonus = fighter.stats.agility / 15; // Linear scaling, no cap
            const dodgeDuration = CONFIG.BATTLE.dodgeDuration * (1 + agilityBonus * 0.3); // Scales with agility
            const dodgeSpeed = speed * (3 + agilityBonus * 1.5); // Faster dodge with more agility
            
            fighter.dodgeEndTime = now + dodgeDuration;
            fighter.stateEndTime = now + 400;
            
            // Intelligence affects dodge direction accuracy (no cap)
            const intelligence = fighter.stats.intelligence || 10;
            const smartness = intelligence / (intelligence + 15); // Diminishing returns, no cap
            const randomness = (1 - smartness) * Math.PI * 0.8;
            
            let dodgeAngle;
            
            // Use pre-calculated escape angle for corner dodges
            if (decision.reason === 'corner' && decision.escapeAngle !== undefined) {
              dodgeAngle = decision.escapeAngle;
            } else if (decision.dodgeAngle !== undefined) {
              // Use pre-calculated dodge angle from AI decision
              dodgeAngle = decision.dodgeAngle;
            } else if (decision.reason === 'projectile') {
            ...
            if (smartness > 0.5) {
              if (testX < margin || testX > CONFIG.BATTLE.arenaWidth - margin ||
                ...
                // Flip dodge direction or dodge toward center
                const centerAngle = Math.atan2(
                  CONFIG.BATTLE.arenaHeight / 2 - fighter.y,
                  CONFIG.BATTLE.arenaWidth / 2 - fighter.x
                );
                dodgeAngle = centerAngle + (Math.random() - 0.5) * 0.5;
              }
            }
        ...
        case 'charge':
          // AGGRESSIVE charge toward enemy (melee monsters)
          fighter.state = 'charging';
          const chargeDx = enemy.x - fighter.x;
          const chargeDy = enemy.y - fighter.y;
          const chargeDist = Math.sqrt(chargeDx * chargeDx + chargeDy * chargeDy) || 1;
          // Faster than normal approach, with acceleration
          const chargeSpeed = speed * (decision.aggressive ? 0.5 : 0.4);
          fighter.vx += (chargeDx / chargeDist) * chargeSpeed;
          fighter.vy += (chargeDy / chargeDist) * chargeSpeed;
          fighter.facing = chargeDx > 0 ? 1 : -1;
          break;
          
        case 'stayClose':
          // Stay close to enemy while waiting for attack cooldown (melee)
          fighter.state = 'moving';
          const stayAngle = Math.atan2(enemy.y - fighter.y, enemy.x - fighter.x) + 
                           (Math.PI / 3) * decision.direction; // Smaller angle = stays closer
          fighter.vx += Math.cos(stayAngle) * speed * 0.25;
          fighter.vy += Math.sin(stayAngle) * speed * 0.25;
          fighter.facing = enemy.x > fighter.x ? 1 : -1;
          break;
          
        case 'kite':
          // RANGED: Move away while staying in attack range
          fighter.state = 'kiting';
          const kiteAngle = Math.atan2(fighter.y - enemy.y, fighter.x - enemy.x);
          const kiteSpeed = speed * 0.35;
          fighter.vx += Math.cos(kiteAngle) * kiteSpeed;
          fighter.vy += Math.sin(kiteAngle) * kiteSpeed;
          fighter.facing = enemy.x > fighter.x ? 1 : -1;
          break;
          
        case 'approachCautious':
          // RANGED: Approach but stop at safe distance
          fighter.state = 'moving';
          const cautionDx = enemy.x - fighter.x;
          const cautionDy = enemy.y - fighter.y;
          const currentDist = Math.sqrt(cautionDx * cautionDx + cautionDy * cautionDy) || 1;
          const targetDist = decision.targetDistance || CONFIG.BATTLE.projectileRange * 0.6;
          
          // Only approach if further than target distance
          if (currentDist > targetDist) {
            fighter.vx += (cautionDx / currentDist) * speed * 0.25;
            fighter.vy += (cautionDy / currentDist) * speed * 0.25;
          }
          fighter.facing = cautionDx > 0 ? 1 : -1;
          break;
        
        case 'escapeCorner':
          // Smart monsters escape from corners/walls toward center
          fighter.state = 'moving';
          const escapeAngle = decision.escapeAngle || Math.atan2(
            CONFIG.BATTLE.arenaHeight / 2 - fighter.y,
            CONFIG.BATTLE.arenaWidth / 2 - fighter.x
          );
          fighter.vx += Math.cos(escapeAngle) * speed * 0.5;
          fighter.vy += Math.sin(escapeAngle) * speed * 0.5;
          fighter.facing = Math.cos(escapeAngle) > 0 ? 1 : -1;
          break;
          
        case 'chargeAngled':
          // Smart melee charge that avoids walls
          fighter.state = 'charging';
          const chargeToDx = enemy.x - fighter.x;
          const chargeToDy = enemy.y - fighter.y;
          const chargeToAngle = Math.atan2(chargeToDy, chargeToDx);
          // Adjust angle slightly toward center if near wall
          const centerAngle = Math.atan2(
            CONFIG.BATTLE.arenaHeight / 2 - fighter.y,
            CONFIG.BATTLE.arenaWidth / 2 - fighter.x
          );
          // Blend toward center (30%) to avoid walls
          const adjustedAngle = chargeToAngle * 0.7 + centerAngle * 0.3;
          const angledChargeSpeed = speed * 0.45;
          fighter.vx += Math.cos(adjustedAngle) * angledChargeSpeed;
          fighter.vy += Math.sin(adjustedAngle) * angledChargeSpeed;
          fighter.facing = chargeToDx > 0 ? 1 : -1;
          break;
          
        case 'wander':
          // Dumb random movement
          fighter.state = 'moving';
      ...
      // Arena edge awareness based on intelligence (no cap)
      ...
      const edgeAwareness = intelligence / (intelligence + 15); // Diminishing returns, no cap
      const edgeMargin = 30 + (edgeAwareness * 70); // 30-100+ pixels from edge
      const edgeForce = 0.2 + (edgeAwareness * 0.8); // How strongly to push away from edge
    ...
    newState.fighters.forEach(fighter => {
      ...
      const intelligence = fighter.stats.intelligence || 10;
      const smartBounce = intelligence / (intelligence + 30) * 0.5; // No cap, approaches 0.5
    ...
    <div className="relative flex flex-col items-center w-full">
          ...
          <div className="w-full max-w-xs mb-2 px-2">
            <div className="flex items-center justify-between gap-2">
              <div className="flex flex-col">
                <div className="text-xs text-orange-400">{fighters[1].monster.trainerName || 'Wild'}</div>
            ...
            return (
              <div key={fighter.monster.id} className={`${isEliminated ? 'opacity-40' : ''}`}>
                <div className="flex items-center justify-between gap-1">
                  <div className="flex flex-col">
                    <div className={`text-xs truncate ${isPlayer ? 'text-blue-400' : 'text-orange-400'}`}>
                      {fighter.monster.trainerName || (isPlayer ? 'You' : 'Wild')}
                    </div>
      ...
      >
        ...
        {fighters.map((fighter) => {
          ...
          const isPlayer = !fighter.monster.isEnemy;
          return (
            <div
            ...
            >
              {/* Trainer name label above monster */}
              <div 
                className={`absolute left-1/2 -translate-x-1/2 text-xs font-bold px-2 py-0.5 rounded whitespace-nowrap ${
                  isPlayer ? 'bg-blue-500/80 text-white' : 'bg-red-500/80 text-white'
                }`}
...
const EnemyPreviewModal = ({ playerMonster, enemy, rivalData, playerName, onStartBattle, onClose }) => {
  ...
  const playerWinsPower = playerPower > enemyPower;
  const enemyWinsPower = enemyPower > playerPower;
  ...
  // Check if this is the #1 nemesis
  const isNemesis = rivalData?.isNemesis && rivalData?.encounters >= 3;
  
  // Generate trash talk for rivals (only once, stored in ref)
  const trashTalkRef = React.useRef(rivalData ? Utils.generateTrashTalk(rivalData, playerName) : null);
  const trashTalk = trashTalkRef.current;
  
  // Auto-hide trash talk after 5 seconds
  React.useEffect(() => {
    if (trashTalk) {
      const timer = setTimeout(() => setShowTrashTalk(false), 5000);
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      {/* Sliding Trash Talk Notification */}
      {trashTalk && (
        <div 
          className={`fixed top-0 left-0 right-0 z-60 transition-transform duration-500 ${
            showTrashTalk ? 'translate-y-0' : '-translate-y-full'
          }`}
        >
          <div className={`mx-4 mt-4 p-4 rounded-2xl shadow-lg ${
            isNemesis 
              ? 'bg-gradient-to-r from-red-600 to-purple-600 border-2 border-red-400' 
              : 'bg-gradient-to-r from-purple-600 to-pink-600 border border-purple-400'
          }`}>
            <div className="flex items-start gap-3">
              <div className="text-3xl">{enemyTrainerAvatar}</div>
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <span className="font-bold text-white">{enemy.trainerName}</span>
                  {isNemesis && <span className="text-xs bg-red-500 px-2 py-0.5 rounded-full">NEMESIS</span>}
                </div>
                <p className="text-white/90 text-sm">"{trashTalk}"</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-gray-900 rounded-3xl p-6 max-w-lg w-full">
        ...
        <div className={`text-center mb-4 rounded-2xl p-4 ${isNemesis 
          ? 'bg-gradient-to-r from-red-900/50 to-purple-900/50 border-2 border-red-500/50' 
          ...
          {isNemesis ? (
            <>
              <p className="text-sm text-red-400 font-bold">🔥 YOUR NEMESIS 🔥</p>
              {rivalData && (
                <p className="text-xs text-gray-400 mt-1">
                  Record: {rivalData.losses}W - {rivalData.wins}L ({rivalData.encounters} battles)
                </p>
              )}
            </>
          ) : enemy.isRivalRematch ? (
        ...
        <div className="flex items-center justify-around mb-4">
          ...
          <div className="text-center">
            ...
            <div className={`text-sm mt-1 font-bold ${playerWinsPower ? 'text-green-400' : enemyWinsPower ? 'text-red-400' : 'text-yellow-400'}`}>
              {playerPower} PWR {playerWinsPower && '▲'}
            </div>
          </div>
          ...
          <div className="text-center">
            ...
            <div className={`text-sm mt-1 font-bold ${enemyWinsPower ? 'text-green-400' : playerWinsPower ? 'text-red-400' : 'text-yellow-400'}`}>
              {enemyPower} PWR {enemyWinsPower && '▲'}
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-800/50 rounded-xl p-3 mb-4">
          <div className="grid grid-cols-3 gap-2 text-xs mb-2">
            <div className="text-center text-blue-400 font-bold">You</div>
            <div className="text-center text-gray-400 font-bold">Stat</div>
            <div className="text-center text-orange-400 font-bold">Enemy</div>
          </div>
        ...
        <div className="bg-gray-800/50 rounded-xl p-2 mb-4">
          <p className="text-xs text-gray-400 text-center">
            Win: <span className="text-yellow-400">{CONFIG.LEVELING.expPerWin} EXP</span> + <span className="text-yellow-400">1🪙</span>
            <br />
            <span className="text-purple-400">Level Up: +{CONFIG.LEVELING.trainingPointsPerLevel} Training Points</span>
          </p>
        </div>
...
const BattleResultModal = ({ result, onClose, onBattleAgain }) => {
  const { won, monster, expGained, trainingPointsGained, leveledUp, levelsGained = 0,
        ...
        <div className="bg-gray-900 rounded-3xl p-6 max-w-md w-full">
          ...
          {won ? (
            <>
              ...
              {/* Level Up - Shows training points earned */}
              {leveledUp && (
                <div className="bg-purple-500/20 border border-purple-500/30 rounded-xl p-4 mb-4 text-center">
                  <h4 className="text-lg font-bold text-purple-400 mb-2">⬆️ LEVEL UP!</h4>
                  {levelsGained > 1 && (
                    <p className="text-purple-300 text-sm mb-2">+{levelsGained} Levels!</p>
                  )}
                  <div className="bg-gray-800/50 rounded-lg p-3">
                    <div className="text-2xl text-purple-400 font-bold">+{trainingPointsGained}</div>
                    <div className="text-xs text-gray-400">Training Points</div>
                    <p className="text-xs text-gray-500 mt-2">Spend in Training tab to boost stats!</p>
                  </div>
                </div>
              )}
            </>
            ...
            <div className="bg-gray-800/50 rounded-xl p-4 mb-4 text-center">
              ...
              {leveledUp && (
                <div className="mt-3 bg-purple-500/20 border border-purple-500/30 rounded-lg p-3">
                  ...
                  <div className="text-lg text-purple-400 font-bold">+{trainingPointsGained} Training Points</div>
                </div>
              )}
            </div>
          )}
          ...
          <div className="bg-gray-800/50 rounded-xl p-4 mb-4">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className={`text-sm font-bold ${
                monster.happiness === 0 ? 'text-red-500' :
                monster.happiness < 20 ? 'text-orange-400' : 'text-green-400'
              }`}>
                {Math.round(monster.happiness || 100)}%
              </span>
            </div>
            <div className="h-3 bg-gray-700 rounded-full overflow-hidden">
              <div 
                className={`h-full transition-all ${
                  monster.happiness === 0 ? 'bg-red-500' :
                  monster.happiness < 20 ? 'bg-orange-500' :
                  monster.happiness < 50 ? 'bg-yellow-500' : 'bg-green-500'
                }`}
                style={{ width: `${monster.happiness || 100}%` }}
              />
            </div>
            {/* Happiness Warning */}
            {monster.isDepressed && (
              <div className="mt-2 p-2 bg-red-500/20 border border-red-500/30 rounded-lg">
                <p className="text-red-400 text-xs text-center font-bold">
                  😢 DEPRESSED! Happiness will recover over time. Cannot battle until 100%.
                </p>
              </div>
            )}
            {!monster.isDepressed && monster.happiness < 20 && (
              <div className="mt-2 p-2 bg-orange-500/20 border border-orange-500/30 rounded-lg">
                <p className="text-orange-400 text-xs text-center">
                  😔 Your monster is sad. Feed and play with it to cheer it up!
                </p>
              </div>
            )}
          </div>
...
export default function MonsterGame() {
  ...
  // Trash talk notification state
  const [trashTalkNotification, setTrashTalkNotification] = useState(null);
  
  const battleRef = useRef(null);
  const careEmotionTimeoutRef = useRef(null);
  const saveTimeoutRef = useRef(null);
  const trashTalkTimeoutRef = useRef(null);
  ...
  // Happiness recovery tick - depressed monsters recover passively
  useEffect(() => {
    const interval = setInterval(() => {
      setMonsters(prev => prev.map(m => {
        // Get current happiness (default to 100 for new monsters, but respect 0 for depressed)
        const currentHappiness = typeof m.happiness === 'number' ? m.happiness : 100;
        
        // Always try to recover if below 100
        if (currentHappiness < 100) {
          // Depressed monsters recover faster (they're resting and being cared for)
          const recoveryRate = m.isDepressed 
            ? CONFIG.HAPPINESS.recoveryPerTick * 2.5  // 2.5x recovery when depressed
            : CONFIG.HAPPINESS.recoveryPerTick;
          const newHappiness = Math.min(100, currentHappiness + recoveryRate);
          
          // Cure depression when happiness reaches 100%
          const stillDepressed = m.isDepressed && newHappiness < CONFIG.HAPPINESS.depressionRecoveryRequired;
          
          console.log(`${m.name}: happiness ${currentHappiness} -> ${newHappiness}, depressed: ${stillDepressed}`);
          
          return { ...m, happiness: newHappiness, isDepressed: stillDepressed };
        }
        return m;
      }));
    }, 3000); // Every 3 seconds (faster recovery)
  ...
  // Helper to generate enemy with chance for any rival to appear
  // Rivals are trainers we've battled before - now they're back!
  const generateEnemyWithRivalChance = (playerMonster, powerMultiplier = 1.0, rivalChance = 0.40) => {
    const rivals = trainerProfile.rivals;
    if (rivals.length === 0) {
      return EnemyFactory.generateMatchedEnemy(playerMonster, powerMultiplier);
    }
    
    // Get #1 nemesis by INTENSITY
    const topRival = rivals.reduce((top, r) => 
      (r.intensity || r.encounters) > (top.intensity || top.encounters) ? r : top, 
      rivals[0]);
    
    // Roll for rival appearance
    if (Math.random() < rivalChance) {
      // Weighted selection based on intensity - higher intensity = more likely to appear
      const totalIntensity = rivals.reduce((sum, r) => sum + (r.intensity || r.encounters), 0);
      let roll = Math.random() * totalIntensity;
      let selectedRival = topRival; // Default to nemesis
      
      for (const rival of rivals) {
        roll -= (rival.intensity || rival.encounters);
        if (roll <= 0) {
          selectedRival = rival;
          break;
        }
      ...
      const enemy = EnemyFactory.generateFromRival(selectedRival, playerMonster, powerMultiplier);
      const isNemesis = selectedRival.id === topRival.id;
      enemy.rivalData = { ...selectedRival, isNemesis };
      return enemy;
    }
  ...
  // Get current rival data for enemy (if it's a rival rematch)
  const getCurrentRivalData = (enemy) => {
    if (!enemy?.isRivalRematch || !enemy?.trainerId) return null;
    const rivalFromProfile = trainerProfile.rivals.find(r => r.id === enemy.trainerId);
    if (rivalFromProfile) {
      // Check if this is the #1 nemesis by INTENSITY
      const topRival = trainerProfile.rivals.length > 0 
        ? trainerProfile.rivals.reduce((top, r) => 
            (r.intensity || r.encounters) > (top.intensity || top.encounters) ? r : top, 
            trainerProfile.rivals[0])
        : null;
      const isNemesis = topRival && topRival.id === rivalFromProfile.id;
      return { ...rivalFromProfile, isNemesis };
    }
    return enemy.rivalData;
  };
  ...
  const handleEndBattle = () => {
    ...
    if (battleState?.winner) {
      ...
      if (isPlayerWin && winnerMonster) {
        ...
        // Award experience and coins (training points come from level ups only)
        ...
        let updatedMonster = expResult.monster;
        ...
        setTrainerProfile(prev => {
          ...
          // Decay intensity for all rivals (battles become "old news")
          updatedRivals = updatedRivals.map(r => ({
            ...r,
            intensity: Math.max(0, (r.intensity || r.encounters) * 0.95), // 5% decay per battle
          }));
          ...
          if (enemyTrainerId) {
            ...
            if (existingRival) {
              ...
              // Intensity boost: +2 base, +1 bonus if battled recently (within last 5 battles)
              const recentBonus = (now - (existingRival.lastBattleTime || 0)) < 300000 ? 1 : 0; // 5 min
              const newIntensity = (existingRival.intensity || existingRival.encounters) + 2 + recentBonus;
              ...
              // Add new rival immediately (trainers become rivals after 1 battle!)
              updatedRivals.push({
                id: enemyTrainerId,
                ...
                losses: 0,
                encounters: 1,
                intensity: 2, // Starting intensity
                lastBattleTime: now,
                lastMonsterName: enemyFighter?.monster.name || 'Unknown',
              });
            }
            
            // Sort by intensity (highest intensity = #1 nemesis)
            updatedRivals.sort((a, b) => (b.intensity || b.encounters) - (a.intensity || a.encounters));
          }
        ...
        setBattleResult({
          ...
          expGained: expAmount,
          trainingPointsGained: expResult.trainingPointsGained || 0, // From level up only
          coinsGained: coinAmount,
          leveledUp: expResult.leveledUp,
          levelsGained: expResult.levelsGained || 0,
          oldStats,
          newStats,
        ...
        if (playerMonster) {
          ...
          setTrainerProfile(prev => {
            ...
            // Decay intensity for all rivals
            updatedRivals = updatedRivals.map(r => ({
              ...r,
              intensity: Math.max(0, (r.intensity || r.encounters) * 0.95),
            }));
            
            const existingRival = updatedRivals.find(r => r.id === enemyTrainerId);
            
            if (existingRival) {
              // Update existing rival - they beat us! (losses boost intensity more - it's personal!)
              const recentBonus = (now - (existingRival.lastBattleTime || 0)) < 300000 ? 1 : 0;
              const newIntensity = (existingRival.intensity || existingRival.encounters) + 3 + recentBonus; // +3 for loss (more intense!)
              ...
              updatedRivals = updatedRivals.map(r => 
                r.id === enemyTrainerId 
                  ? { 
                      ...
                      intensity: newIntensity,
                      lastBattleTime: now,
                      lastMonsterName: enemyFighter?.monster.name,
                    }
                  : r
              );
            } else {
              // Add new rival immediately
              updatedRivals.push({
                id: enemyTrainerId,
                name: enemyTrainerName,
                wins: 0,
                losses: 1,
                encounters: 1,
                intensity: 3, // Higher starting intensity for loss
                lastBattleTime: now,
                lastMonsterName: enemyFighter?.monster.name || 'Unknown',
              });
            }
    ...
    >
      ...
      <div className="max-w-lg mx-auto px-4 py-4 pb-24">
        {view === 'care' ? (
                  ...
                  <div className="mb-4">
                    <div className="flex justify-between text-xs mb-1">
                      <span className="text-gray-400">
                        Happiness
                        {(selectedMonster.happiness || 100) < 100 && (
                          <span className="text-green-400 ml-1 animate-pulse">↑ recovering</span>
                        )}
                      </span>
                    ...
                    <div className="h-2 bg-gray-700 rounded-full overflow-hidden relative">
                      <div 
                        className={`h-full transition-all duration-500 ${selectedMonster.isDepressed ? 'bg-red-500' : 'bg-pink-500'}`}
                        style={{ width: `${selectedMonster.happiness || 100}%` }}
                      />
                      {/* Recovery animation stripe */}
                      {(selectedMonster.happiness || 100) < 100 && (
                        <div 
                          className="absolute top-0 h-full w-2 bg-white/30 animate-pulse"
                          style={{ left: `${selectedMonster.happiness || 0}%` }}
                        />
                      )}
                    </div>
                    {selectedMonster.isDepressed && (
                      <p className="text-xs text-red-400 mt-1">Must reach 100% to battle again</p>
                    )}
                  </div>
                )}
                  ...
                  <>
                    {selectedMonster.isDepressed ? (
                      <div className="text-center py-4">
                        ...
                        <p className="text-gray-500 text-xs">Happiness recovering: {Math.round(selectedMonster.happiness || 0)}%</p>
                        ...
                        <p className="text-xs text-gray-600 mt-2">Recovering automatically... (needs 100%)</p>
                        
                        {/* Cheer Up button - costs coins */}
                        <button
                          onClick={() => {
                            if (trainerProfile.coins >= 5) {
                              setTrainerProfile(prev => ({ ...prev, coins: prev.coins - 5 }));
                              setMonsters(prev => prev.map(m => 
                                m.id === selectedMonster.id 
                                  ? { ...m, happiness: Math.min(100, (m.happiness || 0) + 25) }
                                  : m
                              ));
                              setSelectedMonster(prev => prev ? { ...prev, happiness: Math.min(100, (prev.happiness || 0) + 25) } : prev);
                              triggerEmotion('happy', '💖');
                            }
                          }}
                          disabled={trainerProfile.coins < 5}
                          className={`mt-3 px-4 py-2 rounded-xl font-bold text-sm ${
                            trainerProfile.coins >= 5 
          ...
          <div className="space-y-4">
            ...
            {/* Rivals Section - Show #1 Nemesis prominently */}
            <div className="bg-gray-900/50 rounded-2xl p-4">
              <h3 className="font-bold text-white mb-3">🔥 Your Nemesis</h3>
              {trainerProfile.rivals.length === 0 ? (
                <p className="text-gray-500 text-sm text-center py-4">
                  No rivals yet. Battle trainers to make enemies!
                </p>
              ) : (() => {
                // Get #1 rival by INTENSITY
                const topRival = trainerProfile.rivals.reduce((top, r) => 
                  (r.intensity || r.encounters) > (top.intensity || top.encounters) ? r : top, 
                  trainerProfile.rivals[0]);
                const trainerAvatars = ['👨', '👩', '🧑', '👴', '👵', '🧔', '👱', '👨‍🦰', '👩‍🦰', '🧑‍🦱'];
                const avatar = trainerAvatars[Math.abs(topRival.id?.charCodeAt(0) || 0) % trainerAvatars.length];
                const playerWins = topRival.losses;
                const playerLosses = topRival.wins;
                const intensity = Math.round(topRival.intensity || topRival.encounters);
                
                return (
                  <div className="bg-gradient-to-r from-red-900/50 to-purple-900/50 border border-red-500/30 rounded-xl p-4">
                    <div className="flex items-center gap-4">
                      <div className="text-4xl">{avatar}</div>
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-red-400 font-bold">🔥 NEMESIS</span>
                        </div>
                        <h4 className="text-xl font-bold text-white">{topRival.name}</h4>
                        <p className="text-gray-400 text-sm">{topRival.encounters} battles • {intensity} rivalry intensity</p>
                      </div>
                    </div>
                    ...
                    <div className="mt-2 mb-3">
                      ...
                      <div className="h-2 bg-gray-700 rounded-full overflow-hidden">
                        <div 
                          ...
                          style={{ width: `${Math.min(100, intensity * 5)}%` }}
                        />
                      </div>
                      <p className="text-xs text-gray-600 mt-1">Intensity decays if you don't battle them!</p>
                    </div>
                    
                    {/* Win/Loss record */}
                    <div className="flex justify-around bg-black/20 rounded-lg p-2">
                      <div className="text-center">
                        <div className="text-xl font-bold text-green-400">{playerWins}</div>
                        <div className="text-xs text-gray-500">Your Wins</div>
                      </div>
                      <div className="text-center">
                        <div className="text-xl font-bold text-red-400">{playerLosses}</div>
                        <div className="text-xs text-gray-500">Your Losses</div>
                      </div>
                      <div className="text-center">
                        <div className={`text-xl font-bold ${playerWins > playerLosses ? 'text-green-400' : playerWins < playerLosses ? 'text-red-400' : 'text-yellow-400'}`}>
                          {playerWins > playerLosses ? '👑' : playerWins < playerLosses ? '😤' : '🤝'}
                        </div>
                        <div className="text-xs text-gray-500">
                          {playerWins > playerLosses ? 'Winning!' : playerWins < playerLosses ? 'Losing...' : 'Tied'}
                        </div>
                      </div>
                    </div>
                    
                    {topRival.lastMonsterName && (
                      <p className="text-xs text-gray-500 mt-2 text-center">
                        Last seen with: {topRival.lastMonsterName}
                      </p>
                    )}
                  </div>
                );
              ...
              {/* Other rivals count */}
              {trainerProfile.rivals.length > 1 && (
                <p className="text-gray-500 text-xs text-center mt-3">
                  +{trainerProfile.rivals.length - 1} other trainer{trainerProfile.rivals.length > 2 ? 's' : ''} battled
                </p>
              )}
            </div>
      ...
      {currentEnemy && selectedFighter && !ffaEnemies && (
        <EnemyPreviewModal
          ...
          enemy={currentEnemy}
          rivalData={getCurrentRivalData(currentEnemy)}
          playerName={trainerProfile.name}
